...
function initCurvedSlider(services) {
    ...
    const RADIUS = window.innerWidth > 768 ? 700 : 500;
    const DRAG_SENSITIVITY = -0.003; // Inverted to fix mirror mode after card ordering flip
    const FRICTION = 0.94;
    ...
    let isUpdating = !isMobile; // Disable per-frame updates on mobile
    let lastMouseX = 0;
    ...
    if (isMobile) return; // Exit early for mobile - CSS handles the vertical stack
